...
- 函数按照数据类型分类。结构体形式调用，提示友好，用法清晰。 增加了 分布式锁、整数、浮点数、布尔、JSON、结构体 等多种形式，使用 `rds.NewXXX` 去查看和使用。
- 泛型。存储和获取时自动转换。
...
- `Mutex` 分布式锁（可重入）
  ...
  - 示例代码
    ...
    // 业务逻辑...
    ```
    > 方式二：阻塞。尝试获取，拿不到重试，直到：拿到锁true/上下文超时false/上个锁到期true(避免这种情况。锁的TTL是60秒，如果处理事务可能超过60秒且上下文没有超时设置或大于60秒，应该考虑开定时器去再次加锁可重入，以延长过期时间)。
    ```go
    mu := rds.NewMutex(ctx, "order:"+uuid) 
    if !mu.Lock() {
